...
beta/sw.js            Beta-scoped service worker
...
Classic and Beta have separate service workers. Do not modify the Classic
worker for a Beta-only release. Add new Beta assets to `beta/sw.js`; Beta
navigation must fall back to `./index.html` within the `/beta/` scope, never the
Classic document. Bump the owning worker's cache whenever its pre-cached shell
changes and update cache assertions without weakening offline coverage.
